...
- `demo requirements and release prep`
  - focus on demo scenario, release gates, and allowed pre-release fix scope on
...
- staged room-open waves remain a visible product rough edge and now serve as
  one input into later demo requirements and the post-demo migration track
...
- an earlier room-open UI/status experiment was rolled back to the stable
  baseline `40e0594`;
- current chapter truth now prioritizes demo requirements and release discipline
  over architecture execution
...
- current active planning hotspot is the next demo and its release scope.
...
- the immediate problem is the next demo and its release criteria;
- the runtime migration track is approved but parked until after demo release;
- room-open inspectability is now useful input, not the active chapter by
  itself;
...
- the runtime migration track is approved but not active before demo release;
- the first likely implementation lane after the demo is still room-open
  runtime/diagnostics ownership extraction;
...
- demo work now uses the current architecture as the temporary baseline.
...
Canonical wording for this target now lives in `docs/04_ARCHITECTURE/00_OVERVIEW/ARCHITECTURE.md`.
...
Старые `docs/90_ARCHIVE/02_HISTORICAL_BASELINE/refactor-audit.md` и `docs/90_ARCHIVE/02_HISTORICAL_BASELINE/refactor-plan.md` полезны как historical baseline и карта чувствительных зон.
...
- analysis note: [creator-color-fallback-analysis-2026-04-14.md](../90_ARCHIVE/01_AUDITS/creator-color-fallback-analysis-2026-04-14.md)
