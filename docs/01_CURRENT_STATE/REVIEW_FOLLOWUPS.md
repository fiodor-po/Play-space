...
- `expected_action`: keep the split command layout in place, then triage current red corridor expectations, especially design-system state deltas, image recovery corridors, and token count assumptions after the one-token invariant was removed
- `current_result`: local validation commands are now split into `smoke:e2e` for the short demo gate, `test:e2e:runtime` for room-corridor coverage, `test:e2e:media` for media diagnostics, `test:e2e:design-system` for DOM state deltas, and `test:e2e:all` for the full suite; fresh local proof on 2026-04-23 still shows red runtime/media/design-system expectations, so command hygiene is closed but expectation triage remains
