...
### Context
После closure room-document replica track и перехода к room-open observability
стало ясно, что проекту нужен один явный target runtime model для следующих
architecture phases.

Проблема уже читалась стабильно:

- `BoardStage` остаётся runtime hotspot;
- room-open phase model already exists, but ownership is still too local;
- board document shape stays too loose;
- image binary still sits inside collaborative room content;
- transport, persistence, and runtime ownership still mix too closely.

### Decision / change
Зафиксирован целевой runtime direction:

- one logical room document per room;
- `RoomRuntime` as the room-scoped lifecycle/recovery/diagnostics owner;
- `RoomDocumentV1` as the first typed and versioned shared-content contract;
- current transport adapters stay as the near-term bridge;
- image assets move toward asset references later;
- physical transport unification stays later than runtime/document boundary work.
...
### Why
Проекту нужен был один clear target without reopening broad cleanup now.

Этот decision дал:

- target architecture without forcing a rewrite;
- staged migration order with explicit guardrails;
- clean separation between the current chapter and later runtime work.

### Result
Current planning truth now looks like this:

- active chapter still focuses on room-open observability and diagnostics;
- only the room-open ownership slice is absorbed into the current chapter;
- later runtime phases stay candidate work until explicit planning promotion;
- `Mobile experience` still remains the next chapter in the current roadmap.
...
- `docs/90_ARCHIVE/03_CLOSED_CHAPTERS/board-object-controls-ui-layer/CHAPTER.md`
...
- `docs/03_PRODUCT/01_FLOWS/room-initialization-design.md`
...
### What happened
Был добавлен canonical design doc:
- `docs/04_ARCHITECTURE/03_GOVERNANCE/governance-scaffold-design.md`
...
- `docs/03_PRODUCT/02_SEMANTICS/color-model-design.md` как canonical semantic spec;
...
- handoff между ними должен идти не через надежду на общую память, а через `AGENTS.md`, `docs/01_CURRENT_STATE/ROADMAP.md`, current-context и focused docs.

Потом был сделан documentation cleanup pass на уровне framing:
- `AGENTS.md` больше не должен быть refactor-only guide;
- `docs/01_CURRENT_STATE/ROADMAP.md` должен отражать, что capability checklist в основном собран и проект движется к first hosted alpha;
